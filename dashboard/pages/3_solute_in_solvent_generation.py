...
st.set_page_config(
    page_title="Martignac Solute-in-solvent generation", page_icon="🧪", layout="wide"
)
...
from martignac.workflows.solute_in_solvent_generation import project

paths_for_streamlit()

st.title("Martignac")

st.subheader("Solute-in-solvent generation")
...
if len(df) == 0:
    st.warning(
        f"No solute-in-solvent generation entry found in database ID '{database_id}' on {'prod' if prod_db else 'test'} server",
        icon="⚠️",
    )
else:
    df = df.rename(
    ...
    df = df.set_index(["solute_name", "solvent_name"])
